# 25Nov19: Update of quantro script to remove NAs so that script can run properly (fp215)
# Original source code: https://github.com/stephaniehicks/quantro/blob/master/R/AllClasses.R

# A class for either numeric or logical
setClassUnion("numericORlogical", c("numeric", "logical"))

#' @title quantro2
#' @description This is the S4 class quantro2 container
#'
#' @exportClass quantro2 
#' 
setClass(Class = "quantro2", 
...
#' 
#' @return A \code{quantro2} S4 class object
...
#' applied. The \code{quantro2} function uses the raw unprocessed high-throughput 
#' data to test for global differences in the distributions across a set of groups. 
#' 
#' The \code{quantro2} function will perform two tests:
#' 
...
#' pd <- pData(flowSorted)
#' 
#' qtest <- quantro2(object = p, groupFactor = pd$CellType)
#' 
#' @rdname quantro2
#' @export

quantro2 <- function(object, groupFactor = NULL, B = 0, qRange = NULL,
...
{ 
    ...
    objectMedians <- apply(object, 2, stats::median, na.rm=T)
